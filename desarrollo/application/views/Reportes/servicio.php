...
<section class="content" >
 ...
 <form role="form" id="al_pre" name="al_pre"  method="POST" target="myIframe" action="<?= site_url('index.php/Reportes/generar_servicio'); ?>">
  <table class="table table-condensed" >
    ...
    <td colspan="">
               <label for="" class="col-xs-2 ">Rango</label>
                 <div class="col-xs-4   ">
                          <select name="rango_servicio" id="rango_servicio" class="form-control input-sm" >
...
<script>
  // datetime picker end
     $(function () {
            $('#servicio_datapiker').datetimepicker({
                // minDate: new Date(),
                format: 'YYYY-MM-DD',
            });
            $('#servicio_datapiker2').datetimepicker({
               // minDate: new Date(),
               format: 'YYYY-MM-DD',
            });
            $("#servicio_datapiker").on("dp.change", function (e) {
                $('#servicio_datapiker2').data("DateTimePicker").minDate(e.date);
            });
            $("#servicio_datapiker2").on("dp.change", function (e) {
                $('#servicio_datapiker').data("DateTimePicker").maxDate(e.date);
            });
        });
